...
const GROUND_HEIGHT = 4000;  // Match the CSS ground height
...
const BOMB_BOOST_SPEED = 500; // Dramatic upward boost
...
const TRAMPOLINE_BOOST = 300; // Higher boost than bombs
...
const UFO_COLLISION_WIDTH = 60;
const UFO_COLLISION_HEIGHT = 60;
...
function CannonLaunchGame({ setIsTimerActive, gamePaused, soundEffects }) {
  ...
  // Convert game coordinates to CSS pixels
  function gameToCSSPosition(gameX, gameY) {
    return {
      left: `${Math.round(gameX * 2)}px`,
      bottom: `${Math.round(-(4000 - gameY))}px`
    };
  }

  // Convert CSS pixels to game coordinates
  function cssToGamePosition(cssLeft, cssBottom) {
    // Remove 'px' and convert to numbers
    const leftPx = parseInt(cssLeft);
    const bottomPx = parseInt(cssBottom);
    
    return {
      x: leftPx / 2,
      y: 4000 + bottomPx  // Since bottom is negative, add it to 4000
    };
  }
  ...
  const physicsLoop = () => {
    ...
    // Log the actual CSS pixels for the projectile
    const cssLeft = Math.round(x * 2);
    const cssBottom = Math.round(-y * 2 + GROUND_HEIGHT);
    console.log('CSS pixels:', {
        left: `${cssLeft}px`,
        bottom: `${cssBottom}px`
    });*/
    ...
    const treeHitIndex = treesRef.current.findIndex(tree => {
        ...
        const treeX = tree.x / 2;
        const distanceX = Math.abs(x - treeX);
        const treeWidth = TREE_COLLISION_WIDTH / 2;
        const scaledHeight = TREE_HEIGHT * tree.scale;
        
        return distanceX < treeWidth && y < (2000 + scaledHeight);
    });
    ...
    const bombHitIndex = bombsRef.current.findIndex(bomb => {
        ...
        const bombX = bomb.x / 2;
        const distanceX = Math.abs(x - bombX);
        
        // Check collision at ground level
        return distanceX < BOMB_COLLISION_WIDTH/2 && y < (GROUND_HEIGHT/2 + 20);
    });
    ...
    const trampolineHitIndex = trampolinesRef.current.findIndex(tramp => {
        ...
        const trampX = tramp.x / 2;
        const distanceX = Math.abs(x - trampX);
        const scaledWidth = TRAMPOLINE_WIDTH * tramp.scale;
        
        return distanceX < scaledWidth/2 && y < (2000 + 30); // 30 is height
    });
    ...
    if (holeHitIndex !== -1) {
        if (!fallingInHoleRef.current) {  // Only trigger once when first hitting hole
            ...
            vx = 0;     // Stop forward movement completely (instead of vx *= 0.3)
            ...
            // Keep x position fixed while falling
            vx = 0;
        }
    ...
    // Check UFO collisions using bounding box approach
    const ufoHitIndex = updatedUfos.findIndex(ufo => {
        if (ufo.hasHit) return false;

        // Convert UFO CSS position to game coordinates
        const ufoGamePos = cssToGamePosition(`${ufo.x}px`, `${ufo.y}px`);
        
        // Calculate absolute distances in game coordinates
        const dx = Math.abs(x - ufoGamePos.x);
        const dy = Math.abs(y - Math.abs(ufoGamePos.y));

        console.log('dx:', dx, 'dy:', dy);

        // Check if within collision bounds (UFO_COLLISION_WIDTH/HEIGHT are in game coordinates)
        return (dx < UFO_COLLISION_WIDTH/2) && (dy < UFO_COLLISION_HEIGHT/2);
    });
    ...
    if (ufoHitIndex !== -1) {
        ...
        if (soundEffects) {
            soundEffects.play('gameAction');
            soundEffects.play('lazer');  // Keep 'lazer' instead of 'boing' to match existing code
        }
        tryChangeEmoticon('🛸');
    }
        ...
        >
          ...
          {(gameState === 'flying' || gameState === 'done') && (
            <div
              ...
              style={{
                transform: `translate(${position.x * 2}px, ${-position.y * 2 + GROUND_HEIGHT}px)
                            rotate(${projRotation + 180}deg)`,
              }}
            >
