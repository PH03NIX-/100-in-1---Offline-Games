...
const PRIZES = [
    { value: 30, type: 'seconds', label: '30 SEC', probability: 15, color: '#E74C3C' },
    { value: 3, type: 'minutes', label: '3 MIN', probability: 15, color: '#D4A5A5' },
    { value: 3, type: 'minutes', label: '3 MIN', probability: 15, color: '#FF6B6B' },
    ...
    { value: 7, type: 'minutes', label: '7 MIN', probability: 5, color: '#45B7D1' },
    { value: 10, type: 'minutes', label: '10 MIN', probability: 3, color: '#4ECDC4' },
    { value: 20, type: 'minutes', label: '20 MIN', probability: 1, color: '#96CEB4' },
    { value: 1440, type: 'day', label: <span>FREE<br/>DAY!</span>, probability: 1, color: '#2ECC71' }
];
...
const SpinWheel = ({ onComplete, onClose, soundEffects }) => {
    ...
    const spinWheel = () => {
        ...
        // Start sound effects
        let interval = 50; // Start playing every 50ms
        let soundInterval;

        const playTickSound = () => {
            soundEffects.play('gameAction');
            interval += 20; // Increase interval each time

            if (interval <= 500) {
                // Schedule next tick with increased delay
        ...
        // Use shuffledPrizes instead of PRIZES
        ...
        const baseRotation = 1440 + (210 - ((prizeIndex * segmentSize) + (segmentSize / 2)));
        
        setRotation(baseRotation);
        setPrize(selectedPrize);

        setTimeout(() => {
            clearTimeout(soundInterval); // Clear spinning sound timeout
            soundEffects.play('success'); // Play initial success sound
            ...
            } else if (selectedPrize.type === 'day') {
                totalSeconds = selectedPrize.value * 60; // Already in minutes
            }
