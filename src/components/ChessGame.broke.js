...
const ChessGame = ({ playtime, gamePaused, setIsTimerActive, soundEffects }) => {
  ...
  const handleCellClick = (row, col) => {
    if (isGameOver || (gameMode === 'single' && currentPlayer === 'b')) return;

    const piece = board[row][col];
    if (selectedPiece) {
      const [fromRow, fromCol] = selectedPiece;
      if (isValidMove(fromRow, fromCol, row, col)) {
        const newBoard = makeMove(board, { row: fromRow, col: fromCol }, { row, col });
        ...
        if (gameMode === 'single' && currentPlayer === 'w') {
          setTimeout(makeComputerMove, 500);
        }
      } else {
        setSelectedPiece(null);
      ...
      setPossibleMoves(getPossibleMoves(piece, row, col, board));
    ...
    <div className="pre-chess-game">
      {gameMode === null ? (
        <div className="game-mode-selection">
          ...
          <button onClick={() => setGameMode('twoPlayer')}>Two Player</button>
        </div>
      ) : (
